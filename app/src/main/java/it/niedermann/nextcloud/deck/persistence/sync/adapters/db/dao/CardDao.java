...
public interface CardDao extends GenericDao<Card> {
    ...
    @Query("SELECT b.localId FROM card c join stack s on s.localId = c.stackId join board b on s.boardId = b.localId WHERE c.id = :cardRemoteId and c.accountId = (select a.id from account a where a.name = :accountName)")
    LiveData<Long> getLocalBoardIdByCardRemoteIdAndAccountName(long cardRemoteId, String accountName);
}