...
@AnyThread
public class LabelsRepository extends AbstractRepository {

    public LabelsRepository(@NonNull Context context) {
        super(context);
    }
    ...
    public CompletableFuture<Label> updateLabel(@NonNull Label label) {
        return readAccountAndBoardAsync(label.getAccountId(), label.getBoardId())
                .thenComposeAsync(accountAndBoard -> syncScheduler.pushSingleEntity(
                        accountAndBoard.account(),
                        () -> getDataPropagationHelper(accountAndBoard.account()).updateEntity(
                                new LabelDataProvider(null, accountAndBoard.board(), emptyList()),
                                label, accountAndBoard.account())));
    }
    ...
    public CompletableFuture<Label> createLabel(long accountId, @NonNull Label label, long localBoardId) {
        return supplyAsync(() -> {
            ...
            return readAccountAndBoard(accountId, localBoardId);
                ...
                .thenComposeAsync(accountAndBoard -> syncScheduler.pushSingleEntity(
                        accountAndBoard.account(),
                        () -> getDataPropagationHelper(accountAndBoard.account()).createEntity(
    ...
    public CompletableFuture<EmptyResponse> deleteLabel(@NonNull Label label) {
        return readAccountAndBoardAsync(label.getAccountId(), label.getBoardId())
                .thenComposeAsync(accountAndBoard -> syncScheduler.pushSingleEntity(
                        accountAndBoard.account(),
                        () -> getDataPropagationHelper(accountAndBoard.account()).deleteEntity(new LabelDataProvider(null, accountAndBoard.board(), emptyList()), label, accountAndBoard.account())));
    }
