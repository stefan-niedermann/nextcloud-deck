                ...
                try {
                    AccountImporter.onActivityResult(requestCode, resultCode, data, ImportAccountActivity.this, new AccountImporter.IAccountAccessGranted() {
                        ...
                        public void accountAccessGranted(SingleSignOnAccount account) {
                            DeckLog.info("Account access granted for " + account.userId + " (" + account.url + ")");
                            ExecutorServiceProvider.getLinkedBlockingQueueExecutor().execute(() -> {
                                try {
                                    final Account accountToLogin;
                                    if (restoreAccountId != -1) {
                                        DeckLog.info("Restoring data for account " + restoreAccountId);
                                        ...
                                        importAccountViewModel.updateAccountDirectly(accountToLogin);
                                    } else {
                                    ...
                                    final IResponseCallback<Account> loginCallback = new IResponseCallback<>() {
                                        ...
                                        public void onResponse(Account createdAccount, Headers headers) {
                                            DeckLog.info("Login response received for account: " + createdAccount.getId());
                                            try {
                                                ...
                                                syncManager.refreshCapabilities(new ResponseCallback<>(createdAccount) {
                                                    ...
                                                    public void onResponse(Capabilities response, Headers headers) {
                                                        DeckLog.info("Capabilities refreshed. Server version: " + response.getDeckVersion().getOriginalVersion());
                                                        if (!response.isMaintenanceEnabled()) {
                                                            if (response.getDeckVersion().isSupported()) {
                                                                if (restoreAccountId != -1) {
                                                                    DeckLog.info("Starting database reset for restore...");
                                                                    importAccountViewModel.resetAccountData(restoreAccountId, () -> {
                                                                        DeckLog.info("Reset callback triggered. Saving account and rebooting.");
                                                                        importAccountViewModel.saveCurrentAccountDirectly(createdAccount);
                                                                        runOnUiThreadIfActivityStarted(() -> {
                                                                            restoreWifiPref();
                                                                    ...
                                                                    DeckLog.info("Starting normal sync...");
                                                                        ...
                                                                        public void onResponse(Object response, Headers headers) {
                                                                            var progress$ = syncManager.synchronize(new ResponseCallback<>(account) {
                                                                                ...
                                                                                public void onResponse(Boolean response, Headers headers) {
                                                                                    DeckLog.info("Sync finished successfully.");
                                                                                    restoreWifiPref();
                                                                                    SyncWorker.update(getApplicationContext());
                                                                                ...
                                                                                public void onError(Throwable throwable) {
                                                                                    DeckLog.error("Sync error:", throwable);
                                                                ...
                                                                DeckLog.error("Deck version outdated: " + response.getDeckVersion().getOriginalVersion());
                                                            ...
                                                            DeckLog.warn("Maintenance mode enabled.");
                                                    ...
                                                    @Override
                                                    public void onError(Throwable throwable) {
                                                        DeckLog.error("Refresh capabilities error:", throwable);
                                                        super.onError(throwable);
                                                ...
                                                DeckLog.error("Account not found in Files App:", e);
