...
interface LabelDao : GenericDao<LabelEntity> {
    ...
    @Query("SELECT Label.* FROM Label INNER JOIN JoinCardWithLabel ON Label.localId = JoinCardWithLabel.labelId WHERE JoinCardWithLabel.cardId = :cardId AND JoinCardWithLabel.status != 3")
